...
usage:   Beim ausfÃ¼hren der Datei wird ein GUI-Fenster geÃ¶ffnet, mit dem es mÃ¶glich ist
         ...
         Feature quadratische AuflÃ¶sung
"""
...
layout = [[Text('Transformation eines (Grau-)Bilds (*.bmp, *.png, *.jpg) in eine Datendatei (*.txt) und umgekehrt'),
           Text('')],
          [Text("Welches File?"), FileBrowse(key='path')],
          [Text('Bei Transformation in eine Datendatei: Welche AuflÃ¶sung (quadratisch)?'),
           Listbox([2 ** x for x in range(4, 13)], size=(7, 4), key='resolution')],
    ...
    if values['path'] != None:
        ...
        if path[-1:len(path) - 4:-1] == 'txt':
            ...
            if (size):
                ...
                Popup('Bitte wÃ¤hle zuerst eine AuflÃ¶sung aus.', keep_on_top=True, title="Fehler")
                continue
            ...
            Popup('Bitte wÃ¤hle zuerst ein Bild oder eine Textdatei aus.', keep_on_top=True, title="Fehler")
            continue
